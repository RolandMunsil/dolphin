...
void AI::SaveStateToLog()
{
  ...
  for (auto const& [chunk_coord, chunk] : chunk_coord_to_chunk_map)
  {
    ...
    {
      right_way_values_string << std::fixed << std::setprecision(10) << chunk->going_right_way_actions.ScoreForAction((Action)a) << ";";
    }

    std::ostringstream wrong_way_values_string;
    for (u32 a = 0; a < (u32)Action::ACTIONS_COUNT; a++)
    {
      wrong_way_values_string << std::fixed << std::setprecision(10) << chunk->going_wrong_way_actions.ScoreForAction((Action)a) << ";";
    }

    LogToFileListener("(%i;%i;%i)=R{%s}|W{%s}", chunk_coord.x, chunk_coord.y, chunk_coord.z, right_way_values_string.str().c_str(),
...
void AI::WriteAIParametersToLog()
{
  ...
  LogToFileListener("Reward type: %s", (REWARD_USING_ACCELERATION ? "acc" : "vel"));
  LogToFileListener("Learning method: %s", (MODE_IS_Q_LEARNING ? "Qlearn" : "advupd"));
...
{
  float base_score = player_info_retriever.PlayerSpeed();
  if (REWARD_USING_ACCELERATION)
  {
    base_score -= prevPlayerSpeed;
  }

  return base_score * (player_info_retriever.GoingTheWrongWay() ? -1 : 1);
}
...
bool AI::CheckThatAIIsAlive()
{
  if (player_info_retriever.CrashToRestoreFrameCount() > 0 || player_info_retriever.DuringRestoreFrameCount() > 0)
  {
    ...
    if (!betweenDeathAndRestore)
    {
      LogToFileListener("t=%i exp=%.10f DEATH %i", player_info_retriever.TotalFrames(), CalculateExplorationRate(), restore_count + 1);
    }

    did_q_learning_last_frame = false;
    skip_learning_because_crashing_frame_count++;
    betweenDeathAndRestore = true;
...
void AI::UpdateLapsAndRestoreCount()
{
  if (player_info_retriever.CurrentLapNumber() != prevLapNumber)
  {
    ...
    LogToFileListener("t=%i exp=%.10f LAP %i: %i ms", player_info_retriever.TotalFrames(), CalculateExplorationRate(), prevLapNumber,
                      lapTimeMillis);
...
GCPadStatus AI::GetNextInput(const u32 pad_index)
{
  ...
  if (debug_info_enabled)
  {
    ...
    AILog("Scores: [%7.2f, %7.2f, %7.2f, %7.2f, %7.2f] (no boost)", scoredActions->ScoreForAction(Action::SHARP_TURN_LEFT),
          scoredActions->ScoreForAction(Action::SOFT_TURN_LEFT), scoredActions->ScoreForAction(Action::FORWARD),
          scoredActions->ScoreForAction(Action::SOFT_TURN_RIGHT), scoredActions->ScoreForAction(Action::SHARP_TURN_RIGHT));
    AILog("        [%7.2f, %7.2f, %7.2f, %7.2f, %7.2f] (boost)", scoredActions->ScoreForAction(Action::BOOST_AND_SHARP_TURN_LEFT),
          scoredActions->ScoreForAction(Action::BOOST_AND_SOFT_TURN_LEFT), scoredActions->ScoreForAction(Action::BOOST_AND_FORWARD),
    ...
    AILog("        [%7.2f,        ,        ,        , %7.2f] (drift)", scoredActions->ScoreForAction(Action::DRIFT_AND_SHARP_TURN_LEFT),
